...
namespace OpenCBS.Services
{
    public class LoanServices : Services
    {
        ...
        private List<Installment> AssembleFlatRescheduling(Loan loan, ScheduleConfiguration rescheduleConfiguration,
            IScheduleConfiguration scheduleConfiguration, bool isOlbBased)
        {
            var copyOfLoan = loan.Copy();
            var schedule = copyOfLoan.InstallmentList;
            ...
            var installments = newSchedule as Installment[] ?? newSchedule.ToArray();
            if (installments.Any())
                installments.Last().PaidCapital += overpaidPrincipal;
            ...
            var creditAmount = loan.Amount.Value; //TODO
            var usedDays = 0;
            if (installments.Any())
            {
                usedDays = (rescheduleConfiguration.StartDate - installments.Last().ExpectedDate).Days;
            }
            var daysInYear = scheduleConfiguration.YearPolicy.GetNumberOfDays(rescheduleConfiguration.StartDate);
            var extraInterest = 0 /*creditAmount * scheduleConfiguration.InterestRate / 100 * usedDays / daysInYear*/;
            ...
            if (rescheduleConfiguration.GracePeriod == 0 || rescheduleConfiguration.ChargeInterestDuringGracePeriod)
                firstInterest = creditAmount * rescheduleConfiguration.InterestRate / 100 * daysTillRepayment /
                                daysInYear;

            copyOfLoan.Amount = isOlbBased ? currentOlb : creditAmount; //TODO
            ...
            var rescheduled = SimulateScheduleCreation(copyOfLoan); //TODO
            // Adjust the new schedule's installment numbers
            var increment = installments.Count();
